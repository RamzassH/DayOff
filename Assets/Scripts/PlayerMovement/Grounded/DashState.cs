...
public class DashState : GroundedState
{
    ...
    private CoroutineStarter _coroutine;
    
    public DashState(StateMachine fsm,  MovementData Data, Rigidbody2D rb, GameObject cr) : base(fsm, rb, Data)
    {
        _checkPosition = GameObject.FindWithTag("checkGround").GetComponent<Transform>();
        _checkSize = new Vector2(0.49f, 0.03f);
        _groundLayer = 8;
        _coroutine = (MonoBehaviour.Instantiate(cr) as GameObject).GetComponent<CoroutineStarter>();
    ...
    public override void Enter()
    {
        ...
        _coroutine.StartCoroutine(StartDash(direction));
